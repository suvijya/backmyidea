...
| Issue | Fix |
|---|---|
| Native `<select>` UI breaking styles | Replaced native `<select>` with Radix UI `<Select>` component from `shadcn/ui` in `explore-feed.tsx`. |
| `manifest.json` & Icons 404ing | Added `/manifest.json`, `/icon.png`, `/apple-icon.png`, and `/favicon.ico` to the public allowlist in `src/middleware.ts`. |
